...
export function EventsSection({ events }: { events: Event[] }) {
  ...
  const eventsPerPage = 1 // Default to showing 1 event at a time on mobile
                      ...
                      <CardContent className="p-0">
                        <div className="relative">
                          ...
                          <div className="absolute bottom-0 left-0 right-0 p-4 sm:p-6 md:p-8">
                            ...
                            <p className="text-white/90 mb-4 max-w-2xl text-xs sm:text-sm">
                              {event.description}
                            </p>
      ...
      {totalPages > 1 && (
        <div className="flex justify-center gap-2 mt-6">
          {Array.from({ length: totalPages }).map((_, index) => (
            <button
              key={index}
              className={`h-2.5 w-2.5 rounded-full transition-colors ${currentPage === index ? "bg-primary" : "bg-muted"}`}
