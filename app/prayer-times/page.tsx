...
const formatTime = (timeString: string): string => {
  ...
  return moment(timePart, "HH:mm").format("h:mm A") // Converts the time to the format "h:mm AM/PM"
}
...
export default function PrayerTimesPage() {
  ...
  useEffect(() => {
    const todayRowIndex = prayerTimesData[`${selectedMonth} ${currentYear}`]?.findIndex(
      (day) => Number.parseInt(day.date, 10) === Number.parseInt(currentDate, 10),
    )
    ...
    <div className="flex flex-col min-h-screen bg-gradient-to-b from-green-50 to-white">
      ...
      <section className="relative bg-gradient-to-r from-green-600 to-green-700 text-white overflow-hidden py-20">
        ...
        <div className="container relative py-20 md:py-28 flex flex-col items-center text-center">
          <h1 className="font-heading font-bold text-4xl md:text-5xl lg:text-6xl max-w-4xl mb-6">Prayer Times</h1>
              ...
              <table className="w-full">
                <thead>
                  <tr className="bg-muted">
                    <th className="px-2 py-3 text-left text-xs sm:text-sm">Date</th>
                    <th className="px-2 py-3 text-left text-xs sm:text-sm">Day</th>
                    <th className="px-2 py-3 text-center text-xs sm:text-sm">Fajr</th>
                    <th className="px-2 py-3 text-center text-xs sm:text-sm">Sunrise</th>
                    <th className="px-2 py-3 text-center text-xs sm:text-sm">Dhuhr</th>
                    <th className="px-2 py-3 text-center text-xs sm:text-sm">Asr</th>
                    <th className="px-2 py-3 text-center text-xs sm:text-sm">Maghrib</th>
                    <th className="px-2 py-3 text-center text-xs sm:text-sm">Isha</th>
                  </tr>
                </thead>
                <tbody>
                  {prayerTimesData[`${selectedMonth} ${currentYear}`]?.map((day, index) => (
                    <tr
                      ...
                      style={
                        Number.parseInt(day.date, 10) === Number.parseInt(currentDate, 10) &&
                        selectedMonth === currentMonth
                          ? { backgroundColor: "#0D8A3B" }
                          : {}
                      }
                    >
                      <td className="px-2 py-3 border-t text-xs sm:text-sm">{day.date}</td>
                      <td className="px-2 py-3 border-t text-xs sm:text-sm">{day.day}</td>
                      <td className="px-2 py-3 border-t text-xs sm:text-sm text-center whitespace-nowrap">
                        {day.fajr}
                      </td>
                      <td className="px-2 py-3 border-t text-xs sm:text-sm text-center whitespace-nowrap">
                        {day.sunrise}
                      ...
                      <td className="px-2 py-3 border-t text-xs sm:text-sm text-center whitespace-nowrap">{day.asr}</td>
                      <td className="px-2 py-3 border-t text-xs sm:text-sm text-center whitespace-nowrap">
                        {day.maghrib}
                      </td>
                      <td className="px-2 py-3 border-t text-xs sm:text-sm text-center whitespace-nowrap">
